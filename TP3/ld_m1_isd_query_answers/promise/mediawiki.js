...
MediaWiki.query = function (params) {
    ...
    return MediaWiki.ajax("POST", url).then (function (str) { return JSON.parse(str)});
};
...
MediaWiki.searchImages = function (str) {
    let pReq = MediaWiki.query ( { srsearch : str,
                                   action : "query",
                                   srnamespace : "6",
                                   list : 'search',
                                   utf8 : "1",
    ...
    return pReq.then (function (req) {
        console.log(req);
          let pImagesArray = Promise.all(req.query.search.map((r) => MediaWiki.getImageURL(r.title)));
          return pImagesArray.then ((imagesArray) => imagesArray.map ((json) => json.query.pages[0].imageinfo[0].url));
    });
};
